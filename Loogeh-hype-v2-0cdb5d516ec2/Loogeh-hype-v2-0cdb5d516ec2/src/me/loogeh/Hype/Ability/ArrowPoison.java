...
public class ArrowPoison extends Ability {

	public ArrowPoison() {
		super("Poison Arrow", AbilityInfo.ARROW_POISON);
	}
		...
		if(event.getEntity() instanceof Player) {
			...
			if(member.getClasses().isUseable(getInfo())) {
				...
				if(member.getClasses().getArrowDrawn().equals(getInfo())) {
					...
					event.getProjectile().setMetadata("poisonArrow", new FixedMetadataValue(Main.get(), Boolean.valueOf(true)));
		...
		if(event.getEntity() instanceof Player && event.getDamager() instanceof Arrow) {
			...
			if(!event.isCancelled()) {
				if(event.getDamager().hasMetadata("poisonArrow")) {
					...
					P.forcePotionEffect(player, PotionEffectType.POISON, 60, 1);
				}
			}
		}
	}
