...
public class Tornado {
	...
	public static void spawnTornado(final JavaPlugin plugin, final Location location, final Material material, final byte data, final Vector direction, final double speed, final int amount_of_blocks, final long time, final boolean spew, final boolean explode) {
		...
		if (direction != null) direction.normalize().multiply(speed);
		...
		final int id = new BukkitRunnable() {
			...
			public void run() {
				...
				if (direction != null) location.add(direction);
