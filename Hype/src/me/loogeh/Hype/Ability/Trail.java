	...
	public void onPlayerMoveBlockEvent(PlayerMoveBlockEvent event) {
		...
		if(member.getClasses().isUseable(getInfo())) {
			if(!member.getClasses().hasAbilityElapsed(getInfo())) {
				...
				Location location = location1.toVector().subtract(player.getLocation().getDirection()).toLocation(player.getLocation().getWorld());
						...
						for(int z = cz - r; z < cz + r; z++) {
							...
							if((int) d < r * r) {
								...
								if(block.getType().isBlock()) {
									Block rel = block.getRelative(BlockFace.UP, 1);
									if(rel.getType().equals(Material.AIR) || rel.getType().equals(Material.DEAD_BUSH) || rel.getType().equals(Material.LONG_GRASS)) {
										rel.setType(Material.SNOW);
										if(d < utilMath.r(3)) block.setData((byte) 2);
										else block.setData((byte) 1);
										affectedBlocks.put(rel.getLocation(), System.currentTimeMillis());
									}
								}
							}
						}
