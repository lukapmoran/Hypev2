...
public class ArrowNoxious extends Ability {

	public ArrowNoxious() {
		super("Noxious Arrow", AbilityInfo.ARROW_NOXIOUS);
	}
		...
		if(event.getEntity() instanceof Player) {
			...
			if(member.getClasses().isUseable(getInfo())) {
				...
				if(member.getClasses().getArrowDrawn().equals(getInfo())) {
					...
					event.getProjectile().setMetadata("noxiousArrow", new FixedMetadataValue(Main.get(), Boolean.valueOf(true)));
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		if(event.getEntity() instanceof Player && event.getDamager() instanceof Arrow) {
			...
			if(!event.isCancelled()) {
				if(event.getDamager().hasMetadata("noxiousArrow")) {
					M.message(player, "Ability", ChatColor.WHITE + "You were hit by a " + ChatColor.YELLOW + getName());
					P.forcePotionEffect(player, PotionEffectType.CONFUSION, 60, 2);
				}
			}
		}
	}
